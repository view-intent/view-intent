"use strict";
var __extends = (this && this.__extends) || (function () {
    var extendStatics = Object.setPrototypeOf ||
        ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
        function (d, b) { for (var p in b) if (b.hasOwnProperty(p)) d[p] = b[p]; };
    return function (d, b) {
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
Object.defineProperty(exports, "__esModule", { value: true });
var React = require("react");
var main_1 = require("../../../view-intent/main");
var SelectboxModal = /** @class */ (function (_super) {
    __extends(SelectboxModal, _super);
    function SelectboxModal(props) {
        var _this = _super.call(this, props) || this;
        _this.viewInfo = SelectboxModal.viewInfo;
        _this.state = {};
        return _this;
    }
    SelectboxModal.prototype.render = function () {
        return React.createElement("div", { className: " " + this.props.className },
            React.createElement("button", { onClick: function () { main_1.ViewIntent.back(); } },
                "voltar - ",
                this.viewState.instanceId));
    };
    return SelectboxModal;
}(main_1.View));
exports.SelectboxModal = SelectboxModal;
(function (SelectboxModal) {
    SelectboxModal.viewInfo = {
        area: "viui",
        name: "SelectboxModal",
        type: SelectboxModal,
        frameId: "root",
        require: "stack",
    };
})(SelectboxModal = exports.SelectboxModal || (exports.SelectboxModal = {}));
exports.SelectboxModal = SelectboxModal;
console.log("selectbox modal");
main_1.ViewIntent.registerViewType(SelectboxModal.viewInfo);
//# sourceMappingURL=selectbox-modal.js.map